...
| 에스컬레이션 = "그냥 제일 큰 모델 박기" | [구조 에이전트](core/RESCUE-PROTOCOL.md)는 작업 하나, 최대 2회, 한 번에 축 하나만(모델 *또는* 노력 수준), 진짜 추론/능력 격차일 때만. 모순된 스펙은 재계획으로 라우팅 |
...
3. **진짜 추론/능력 격차 → 구조 에이전트(Rescue Agent)**: 이 작업 하나에만
   ...
   횟수와는 별도로 계산), **한 번에 축 하나만** 올립니다 — `model_capability_gap`이면
   먼저 모델만 올리고 두 번째 시도에서 실패할 때만 노력 수준까지 추가로
   올리고, `reasoning_gap`이면 순서가 반대입니다(먼저 노력 수준, 그다음
   모델). 격리된 마지막 통과 상태에서 시작하며, 명시적인 `forbidden_scope`가
