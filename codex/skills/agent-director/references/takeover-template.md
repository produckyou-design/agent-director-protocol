...
active profile's failure threshold (Codex default: two full failed revision loops), or when the
implementer demonstrably cannot perform the task, and after explicit current-session user
authorization.
...
- `user_authorization_evidence` — the explicit current-session user authorization after the takeover disclosure. A generic request to fix the task is insufficient.
